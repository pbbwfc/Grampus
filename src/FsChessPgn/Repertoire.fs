...
module Repertoire =
    ...
    let mutable rfol = 
        let def = Path.Combine(System.Environment.GetFolderPath(System.Environment.SpecialFolder.MyDocuments),"Grampus\\repertoire")
    ...
    let UpdateWhite () =
        let rec domvt cemvo pbd cbd (imtel:EncodedMoveTextEntry list) (repopts:RepOpts) (repmove:RepMove) = 
                        ...
                        let nrepmove =
                            if repmove.ContainsKey(bdstr) then
                                ...
                                if cro.San<>exro.San then
                                    BlackErrors<-("Duplicate move found for bdstr " + bdstr + " with moves " + cro.San + " and " + exro.San + ".")::BlackErrors
                                repmove
                            else
                                repmove.Add(bdstr,cro)
                        domvt emvo cbd nbd imtel.Tail repopts nrepmove
                |EncodedRAVEntry(mtel) -> 
                    let nrepopts,nrepmove = domvt cemvo pbd cbd mtel repopts repmove
