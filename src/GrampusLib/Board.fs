...
module Board =
    ...
    let SquareAttacksTo (mto : Square) (by : Player) (bd : Brd) =
        (AttacksTo mto by bd) |> Bitboard.ToSquares
    ...
    let FromSimpleStr(str : string) =
        ...
        let col =
            if bits.[1] = "w" then Player.White
            else Player.Black
        
        let pcs =
            let chars = bits.[0].ToCharArray()
            chars
            |> Array.map Piece.Parse
            |> List.ofArray
        
        let fen =
            { emp with WhosTurn = col
                       PieceAt = pcs
                       CastleRights = CstlFlgs.All }
            |> FEN.FromBd
    ...
    let PrintAscii(bd : Brd) =
        for irank = 0 to 7 do
            let getch ifile =
                ...
                else piece |> Piece.PieceToString
